...
if args.make_video or args.make_zoom_video:
    if not os.path.exists('output/steps'):
...
if args.video_style_dir:
    ...
    cwd = os.getcwd()
    args.output = os.path.join(cwd, "output/steps", filename)
...
def checkin(i, losses):
    ...
    TF.to_pil_image(out[0].cpu()).save(f"{filename}_{i}{file_extension}", pnginfo=info)
...
def ascend_txt():
    ...
    if args.make_video:
        ...
        imageio.imwrite('./output/steps/' + str(i) + '.png', np.array(img))
        ...
        while True:
            ...
            if args.make_zoom_video:
                if i % args.zoom_frequency == 0:
                    ...
                    img = np.array(out.mul(255).clamp(0, 255)[0].cpu().detach().numpy().astype(np.uint8))[:,:,:]
                    img = np.transpose(img, (1, 2, 0))
                    imageio.imwrite('./output/steps/' + str(j) + '.png', np.array(img))
...
if args.make_video or args.make_zoom_video:
    ...
    for i in range(init_frame,last_frame):
        temp = Image.open("./output/steps/"+ str(i) +'.png')
        keep = temp.copy()
        frames.append(keep)
        temp.close()
    ...
    if args.output_video_fps > 9:
        ...
        output_file = re.compile('\.png$').sub('.mp4', f"output/{os.path.basename(args.output)}")
        try:
            p = Popen(['ffmpeg',
                       '-y',
                       '-f', 'image2pipe',
